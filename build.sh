...
while true; do
  ...
  if [ -f $CFG ]; then
    STRINGS=$(/var/jb/usr/bin/strings "$CFG" 2>/dev/null)
    echo "strings out: $STRINGS" >> $LOG
    ON=$(echo "$STRINGS" | grep -A1 'scheduleOn' | grep -o '^[01]$' | head -1)
    H=$(echo "$STRINGS" | grep -A1 'hour' | grep -o '^[0-9]*$' | head -1)
    M=$(echo "$STRINGS" | grep -A1 'minute' | grep -o '^[0-9]*$' | head -1)
